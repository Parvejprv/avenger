...
class Board {
    ...
    // Check if there is any slot is available or not
    boolean isSlotAvailable(int row, int col) {
        return this.matrix[row][col] == ' ';
    }
    
    // printing the board that 
    void printBoard() {
// ---------        
// | X O ? |
// | X ? O |
...
// ---------
        ...
        // Printing last line of dashes of the board
        for ( int i = 0 ; i < 9 ; i++ ) {
            System.out.print('-');
        }
        System.out.println();
...
class Game {
    ...
    Game() {
        ...
        System.out.print("Marker of player 1 (Choose between(X,O)) :  ");
    ...
    boolean makeMove() {
        ...
        System.out.println("Please enter the coordinates (row-col), where you want to put your "+currentPlayer.getMarker());
        int row = sc.nextInt();
        int col = sc.nextInt();
...
public class tictactoe_learning {
    public static void main( String args[] ) {

//        Scanner sc = new Scanner(System.in);

        Game newGame = new Game();      // object
        newGame.play();             // call the play maethod here
